...
(baseplot<-ggplot(as.data.frame(y),aes(x=dim1,y=dim2))+geom_point()+geom_density2d())

jmod<-jags.model("finite_mixture.bugs", data = list(y=y,n=nrow(y),d=ncol(y),Nclust=3,Omega0=diag(2)), n.chains=3, quiet=TRUE)
update(jmod, 1000, progress.bar="none")
system.time(samps0<-coda.samples(jmod, variable.names=c("mu_cl","z","probs","Omega"), n.iter=5000, progress.bar="none")) 
...
baseplot+geom_point(aes(x=dim1,y=dim2),data=mu_s,colour="red",shape=2,size=2)
