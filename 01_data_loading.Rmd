...
#compute p-values by genotype instead of by experiment
d1<-d[,c("set_genotype","set_lifespans","ref_lifespans","ranksum_p")]
d1<-d1 %>% group_by(set_genotype) %>% summarise(set_lifespans=merge_lifespans(set_lifespans),ref_lifespans=merge_lifespans(ref_lifespans),max_pval=max(ranksum_p),min_pval=min(ranksum_p))
d1<-d1 %>% mutate(combined_pval=str2pval(set_lifespans,ref_lifespans))
d1<-d1 %>% select(set_genotype,max_pval,min_pval,combined_pval)
write.csv(as.data.frame(d1),file="data/mccormick_all_pvals.csv",row.names=FALSE)
#d<-d %>% left_join(d1,by="set_genotype")
...
sv<-sv %>% group_by(id,experiments,genotype,is_ref) %>% summarise(mean_lifespan=mean(lifespan),median_lifespan=median(lifespan),n=length(lifespan)) %>% transform(genotype=toupper(genotype))
sv1<-subset(sv,is_ref==TRUE)
sv1$is_ref<-NULL
colnames(sv1)[3:6]<-paste0("ref_",colnames(sv1)[3:6])
sv2<-subset(sv,is_ref==FALSE)
sv2$is_ref<-NULL
colnames(sv2)[3:6]<-paste0("set_",colnames(sv2)[3:6])
sv<-sv1 %>% inner_join(sv2,by=c("id","experiments"))
#account for experiment-level random effects
#by using inverse variance weighting to pool across experiments
sv<-sv %>% transform(mean_lifespan_change=set_mean_lifespan-ref_mean_lifespan,ivar_wt=1/(1/set_n+1/ref_n))
sv<-sv %>% group_by(set_genotype) %>% summarise(lifespan_change=sum(mean_lifespan_change*ivar_wt)/sum(ivar_wt))
sv %>% subset(set_genotype=="LOS1") #should be about +7
...
dat<-cbind(sv1[,"lifespan_change"],X)
...
dat2<-cbind(sv2[,"lifespan_change"],X2)
