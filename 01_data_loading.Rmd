...
sv<-sv %>% group_by(genotype) %>% summarise(mean_lifespan=mean(lifespan),median_lifespan=median(lifespan))

#expressed genes vs RLS perturbs
table(rownames(ge) %in% sv$genotype)
table(sv$genotype %in% rownames(ge))
sv1<-sv %>% subset(genotype %in% rownames(ge))
...
table(ge_perts %in% sv$genotype)
table(sv$genotype %in% ge_perts)
sv2<-sv %>% subset(genotype %in% ge_perts)
ge2<-ge[,ge_perts %in% sv2$genotype]
rownames(ge2)<-paste0("expr_",rownames(ge2))
colnames(ge2)<-colData(ge2)$pert_geneSymbol
X2<-t(assay(ge2,"logratio")[,sv2$genotype])
