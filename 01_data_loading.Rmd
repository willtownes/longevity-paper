...
rab<-assay(sce,"relative_abundance")
cm<-colData(sce)
cm2<-data.frame(celltype=cm$cell.type,plate=cm$plate)
cm2$grp<-factor(paste0(cm2$celltype,"_p",cm2$plate))
f<-function(g){
  x<-as.matrix(rab[,cm2$grp==g])
  drop(rowMeans(x))
}
rab2<-vapply(levels(cm2$grp),f,FUN.VALUE=rep(0,nrow(rab)))
write.table(rab2,"./data/worm_scrna_atlas_merged_rel_abund.txt")
```
...
wget -P data https://s3.amazonaws.com/mssm-seq-genemapping/Caenorhabditis_elegans_mapping.rda
```
...
get_gene_info<-function(g,org=c("worm","yeast"),verbose=TRUE){
  ...
  org<-match.arg(org)
  if(verbose){ print(paste("species:",org)) }
  org_match<-list(yeast=c("scerevisiae","ensembl_gene_id"),worm=c("celegans","wormbase_gene"))
...
load_archs4<-function(species=c("worm","yeast"),dir="./data",v="v1"){
  ...
  d<-h5read(hdf5path,"data/expression")
  m<-h5read(hdf5path,"meta")
  i<-h5read(hdf5path,"info")
  cmeta<-do.call(cbind,m[names(m)!="genes"])
  g<-as.character(m[["genes"]])
  rownames(d)<-toupper(as.character(m[["genes"]]))
  SummarizedExperiment(list(counts=d),colData=cmeta,metadata=i)
}
