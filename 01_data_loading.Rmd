...
d_a<-d_a[guniq,1:(ncol(d_a)-3)]
d_m<-d_m[guniq,1:(ncol(d_m)-3)]
d_p<-d_p[guniq,1:(ncol(d_p)-3)]
cnames<-colnames(d_a)
...
sv<-subset(sv,is_ref==FALSE)
#collapse survival to mean or median value for each gene
#ignoring batch effects across experiments, mating_types etc
sv1<-sv %>% group_by(genotype) %>% summarise(mean_lifespan=mean(lifespan),median_lifespan=median(lifespan))
sv1<-sv1 %>% transform(genotype=toupper(genotype))

table(rownames(ge) %in% sv1$genotype)
table(sv1$genotype %in% rownames(ge))

sv1<-sv1 %>% subset(genotype %in% rownames(ge))
ge1<-ge[rownames(ge) %in% sv1$genotype,]
X<-assay(ge1,"logratio")[sv1$genotype,] #sort order match
dat<-cbind(sv1[,2:3],X)
rownames(dat)<-rownames(X)
write.table(dat,file="data/merged_deleteome_mccormick.txt")
```
