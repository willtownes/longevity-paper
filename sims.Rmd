...
K<-15 #number of gene clusters (pathways), avg of G/K genes per pathway
L<-10 #latent dimensional space
b<-c(-5,-5,5,5,5,rep(0,L-5)) #coefficients for survival outcome
...
hand code the phi_k coefficient vector prototypes. 5 of 25 are survival related (have large values in last 5 dimensions)
...
```{r}
perts_surv<-sample.int(G,size=100)
perts_expr<-sample.int(G,size=80)
...
sdat<-list(xx=xx_train, xs=xs_train, Nx=length(xx_train), Ns=length(xs_train), ncell_s=rep_len(1,length(xs_train)), ncell_x=bulk_ncells, L=10, K=15, G=nrow(w_train), W=w_train, y=y_train, dir_wts=rep(1,K), Ls=5)
