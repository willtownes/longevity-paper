...
Comparing model prediction of pro-longevity vs observed change in RLS for McCormick data. Also (below), interpreting the coefficients of GLMNET fit to GO-only.
...
for(sp in species){
  ...
  ofile<-fp(bp,paste0("final_preds_",sp,".txt"))
...
pd2<-merge(subset(pd,species=="yeast"),sv,by.x="gene",by.y="set_genotype")

(plt<-ggplot(pd2,aes(x=prob_pro_longevity,y=lifespan_change))+geom_point(size=.5,alpha=.9)+theme_bw() +xlab("predicted probability gene is pro-longevity") +ylab("change in replicative lifespan after knockout") +geom_smooth(color="blue",size=.5,se=FALSE)+facet_wrap(~ga))
...
for(sp in species){
  ...
  print(best)
  fit<-res[[which(alphas==best$alpha)]]
  ofile<-fp(bp,paste0("go_only_glmnet_fit_",sp,".rds"))
  saveRDS(fit,file=ofile)
}
```

For GO predictors only, the best CV was achieved with alpha=0.15 for worm and alpha=0.1 for yeast. We now extract the most informative predictors by thresholding the coefficients and reporting on the odds ratio scale

```{r}
spp<-c("worm","yeast")
for(sp in spp){
  fit<-readRDS(fp(bp,paste0("go_only_glmnet_fit_",sp,".rds")))
  ...
  res$Effect<-signif(res$Effect,2)
