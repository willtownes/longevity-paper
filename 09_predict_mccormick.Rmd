...
ggplot(pd,aes(x=prob_pro_longevity))+geom_histogram(color="black",fill="white")+facet_grid(ga~species,scales="free_y")+theme_bw()+xlab("predicted probability of pro-longevity")
...
for(sp in species){
  ...
  ofile<-file.path("./results/auto",paste0("topgenes_",sp,".csv"))
  #write.table(res,file=ofile,quote=FALSE,row.names=FALSE)
  write.csv(res,file=ofile,row.names=FALSE)
}
a<-read.csv(file.path("./results/auto",paste0("topgenes_yeast.csv")))
```
