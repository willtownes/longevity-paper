...
preds<-predict(res,Xtst2)

library(naivebayes)
res<-naive_bayes(Xtrn,ytrn,usekernel=TRUE)
preds<-predict(res,Xtst2,type="prob")
res<-train(Xtrn,ytrn,preProcess="nzv",metric="Kappa",trControl=trainControl("repeatedcv",3,repeats=1),method="naive_bayes",tuneGrid=expand.grid(laplace=c(0.5,1),usekernel=TRUE,adjust=c(0.5,1)))
