---
title: "Validate the GO+GXP model predictions on yeast lifespan data"
author: "Will Townes"
date: "11/14/2018"
...
To validate our results, we obtained two auxiliary data sources for yeast: replicative lifespan from McCormick et al 2015 and chronological lifespan from Marek and Korona (2013). We examine whether genes predicted as associated with pro or anti longevity by the GO terms+gene expression model end up having longer/shorter lifespans in these alternative datasets where the different yeast strains had those genes deleted.
...
```{r}
pd<-read.table("./results/auto/final_preds_go_gxp_yeast.txt",header=TRUE)
pd$ga<-"non-genage"
...
ggplot(pd2,aes(x=prob_pro_longevity,y=lifespan_change,colour=genage_category)) +geom_point(alpha=.9) +xlab("predicted probability gene is pro-longevity") +ylab("change in replicative lifespan after knockout") +geom_smooth(color="black",size=.5,se=FALSE) +facet_wrap(~ga) +scale_colour_manual(values=c("red","blue","gray"))
